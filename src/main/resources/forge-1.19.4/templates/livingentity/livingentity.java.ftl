...
public class ${name}Entity extends ${extendsClass} <#if interfaces?size gt 0>implements ${interfaces?join(",")}</#if> {
	...
	<#if data.sensitiveToVibration>
	private final DynamicGameEventListener<VibrationListener> dynamicGameEventListener = new DynamicGameEventListener(new VibrationListener(new EntityPositionSource(this, this.getEyeHeight()), getListenerRadius(), this));
	</#if>
